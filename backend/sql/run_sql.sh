...
for required in DB_HOST DB_PORT DB_NAME DB_SCHEMA DB_APP_USER; do
  eval "value=\${$required:-}"
  if [ -z "$value" ]; then
...
# Schema enforcement: prevents accidental execution against a wrong schema.
ALLOWED_SCHEMA="${SQL_ALLOWED_SCHEMA:-app}"
if [ "$DB_SCHEMA" != "$ALLOWED_SCHEMA" ]; then
  log_error "DB_SCHEMA must be '$ALLOWED_SCHEMA'. Current value: '$DB_SCHEMA'"
  log_error "Override with: SQL_ALLOWED_SCHEMA=<schema_name>"
  exit 1
fi
...
if [ "$PASSWORD_MODE" = "config" ]; then
  ...
  if ! DB_PASSWORD="$(prompt_password_masked "Database password for '$DB_APP_USER' on '$DB_NAME': ")"; then
...
for arg in "$@"; do
  ...
  *)
  ...
  # 00_bootstrap_role_and_db.sql must connect to the bootstrap DB (usually postgres)
  # because the target database may not exist yet.
  db_name="$DB_NAME"
  case "$(basename "$sql_file")" in
  00_bootstrap_role_and_db.sql)
    db_name="${DB_BOOTSTRAP_NAME:-postgres}"
    log_debug "Bootstrap script: connecting to database '$db_name'"
    ;;
  esac
  ...
  if ! psql \
    ...
    --file="$sql_file" >>"$LOG_FILE" 2>&1; then
